...
const BookingPage = () => {
  ...
  const bookingMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      if (product.paymentOption !== "COD") {
        ...
        navigate("/dashboard/my-orders"); // COD → My Orders
      }
    },
    onError: (error) => {
  ...
  const handleBookingSubmit = (e) => {
    ...
    if (quantity > product.quantity) {
      return toast.error(`Cannot order more than available quantity (${product.quantity})`);
    }
  ...
  const handleQuantityChange = (e) => {
    ...
    if (value > product.quantity) {
      toast.error(`Cannot order more than available quantity (${product.quantity})`);
